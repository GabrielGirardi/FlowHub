  ...
  const fetchHackerNews = async (category: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://hacker-news.firebaseio.com/v0/${categoryMap[category]}.json`
      );
      ...
      const stories = await Promise.all(
        limitedIds.map(async (id: number) => {
          ...
          return {
            ...
            urlToImage:  mimic.pages.techNews.fallbackImage,
    ...
    <div className="container mx-auto max-w-7xl px-4">
      ...
      >
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Fique por dentro das últimas notícias e tendências do mundo da
          tecnologia
        </p>
      </motion.div>
      ...
      >
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          <TabsList className="grid w-full md:w-auto grid-cols-3 md:grid-cols-4 h-12 p-1 bg-gradient-to-r from-muted/50 to-muted/30 backdrop-blur-sm border border-border/50">
            {mimic.pages.techNews.categories.map((category) => {
            ...
            {loading ? (
              ...
              savedArticles.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-muted-foreground max-w-md mx-auto">
                    Salve artigos interessantes para lê-los mais tarde. Use o
                    botão de bookmark nos artigos!
                  </p>
                </motion.div>
              ) : (
              ...
              >
                ...
                <p className="text-muted-foreground">
                  Não foi possível carregar as notícias. Tente atualizar a
                  página.
                </p>
              </motion.div>
            ) : (
        ...
        <Card className="border-0 shadow-xl bg-gradient-to-br from-primary/5 via-card to-accent/5 backdrop-blur-sm relative overflow-hidden">
          ...
          <CardContent className="relative z-10">
            <p className="text-muted-foreground leading-relaxed">
              Esta página apresenta as últimas notícias de tecnologia
              diretamente do Hacker News, uma das maiores comunidades de
              tecnologia do mundo. Todas as notícias são atualizadas em tempo
              real e você pode salvar seus artigos favoritos para ler mais
              tarde.
            </p>
    ...
    >
      <Card className="overflow-hidden h-full flex flex-col border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-card to-card/50 backdrop-blur-sm group">
        <div className="aspect-video w-full overflow-hidden relative">
          <img
            src={article.urlToImage || mimic.pages.techNews.fallbackImage}
            alt={article.title}
            className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110"
            onError={(e) => {
              const target = e.target as HTMLImageElement;
              target.onerror = null;
              target.src = mimic.pages.techNews.fallbackImage;
            }}
          />
