...
const FakeData = () => {
  ...
  const [currentType, setCurrentType] = useState<DataTypeOptionValue>("name");
            ...
            <CardContent className="space-y-8">
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3 min-h-[3rem] p-4 bg-muted/30 rounded-xl border-2 border-dashed border-muted">
                  {dataTypes.length > 0 ? (
                    dataTypes.map((type) => {
                      const option = mimic.pages.fakerData.dataOptions.find(
                        (opt) => opt.value === type
                      );
                      return (
                        <div
                ...
                <div className="flex gap-3">
                  <Select
                    value={currentType}
                    onValueChange={(value) =>
                      setCurrentType(value as DataTypeOptionValue)
                    }
                  >
                    ...
                    <SelectContent>
                      {mimic.pages.fakerData.dataOptions.map((option) => (
                        <SelectItem
                          key={option.value}
                          value={option.value}
          ...
          <Card className="xl:row-span-2 border-0 shadow-xl bg-card/50 backdrop-blur-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                {mimic.pages.fakerData.dataOptions.map((type) => (
                  <div
                    key={type.value}
                    className="group p-4 bg-gradient-to-r from-muted/30 to-muted/10 rounded-xl border hover:border-primary/30 transition-all hover:shadow-md"
                  >
                    ...
                    <div className="bg-background/50 p-3 rounded-lg text-sm font-mono text-muted-foreground">
                      {(() => {
                        switch (type.value) {
                          ...
                          case "product":
                            return `${faker.commerce.productName()} - R$${faker.commerce.price()}`;
                          case "image":
              ...
              {generatedData.length > 0 ? (
                <Tabs defaultValue="preview" className="w-full">
                  <TabsList className="mb-6 bg-muted/50 p-1 h-12">
                    <TabsTrigger
                      value="preview"
                      className="h-10 px-6 data-[state=active]:bg-background data-[state=active]:shadow-md"
                    >
                      📋 Preview
                    </TabsTrigger>
                    <TabsTrigger
                          ...
                          <thead className="bg-gradient-to-r from-muted to-muted/70">
                            <tr>
                              {Object.keys(generatedData[0]).map((key) => (
                                <th
                                  key={key}
                                  className="p-4 text-left text-xs uppercase font-bold tracking-wider text-muted-foreground"
                                >
                                  {key}
                                </th>
                              ))}
                ...
                <div className="py-16 text-center">
                  ...
                  <p className="text-muted-foreground max-w-md mx-auto">
                    Configure os tipos de dados e a quantidade, depois clique em
                    "Gerar Dados Falsos" para ver os resultados aqui.
                  </p>
                </div>
              )}
            </CardContent>
